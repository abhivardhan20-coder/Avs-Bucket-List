...
export const AppErrorBoundary: React.FC<AppErrorBoundaryProps> = ({
  ...
  const handleError = useCallback((error: unknown, info: React.ErrorInfo) => {
    reportError(error instanceof Error ? error : new Error(String(error)), { componentStack: info.componentStack || '' }, variant);
    onError?.(error instanceof Error ? error : new Error(String(error)), { componentStack: info.componentStack || '' });
  }, [variant, onError]);
