...
interface Migration {
  id: string;
  run: (user: UserProfile) => Promise<void>;
}

const MIGRATIONS: Migration[] = [
  {
    id: 'v1_normalize_emails',
    run: async (user) => {
...
export async function runMigrations(user: UserProfile): Promise<void> {
  if (!user) return;
  const APPLIED_MIGRATIONS_KEY = 'av_applied_migrations';
  let applied: string[] = [];
  try {
    applied = JSON.parse(localStorage.getItem(APPLIED_MIGRATIONS_KEY) || '[]');
  } catch (e) {
    applied = [];
  }

  // Handle users who already ran the old linear migration
  if (localStorage.getItem('av_migration_v14_done') && applied.length === 0) {
    applied = MIGRATIONS.map(m => m.id);
    localStorage.setItem(APPLIED_MIGRATIONS_KEY, JSON.stringify(applied));
  }
